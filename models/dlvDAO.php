...
class DlvDAO extends ConnectionDAO
{
    ...
    {
        $query = "SELECT p.`prod_name` as `product-name`, cp.`quantity` as `quantity`, cp.`particulars` as `particulars`, cp.`item_id` as `item-id`, cp.`expiry_date` as `exp-date`, cp.`unit_price` as `unit-price`, cp.`selling_price` as `selling-price` FROM `consigned_product` cp LEFT JOIN `product` p ON p.`prod_id` = cp.`prod_id` WHERE cp.`cd_id` = '$cd_id' ";
        try {
            $this->openConnection();
